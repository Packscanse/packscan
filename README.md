...
- **Chain branding**: each store sets its primary color (admin Stores page) and the whole app themes to it — buttons, active tabs, focus rings — so an ICA store is ICA-red and a COOP store COOP-green on the same deployment. Text contrast is picked automatically from the color's luminance.
