...
- **Operations dashboard** (`/admin/operations`): today and last-30-days volumes per store with **carrier share percentages** and **average time-to-pickup**, an hour-of-day staffing profile, shelf aging (overdue pickups + returns awaiting driver collection, live wait averages), a **settlement CSV export** per store × carrier and month — all aggregated in the database, so dashboards cost the same at 1000 stores as at one, and (admins) carrier-event outbox health with re-queue controls.
- **Login rate limiting** per email (10/15 min) and per client IP (30/15 min) — distributed via Upstash Redis (REST) when configured, bounded in-memory fallback for single-instance/dev. **Nonce-based CSP** (`strict-dynamic`) set in middleware. CI (GitHub Actions) runs typecheck, lint, and all three suites against a Postgres service container.
- **PII retention & data-subject rights**: `npm run purge:pii` (cron it) scrubs contact data from terminal packages, notifications, and stale pre-advice; `npm run privacy lookup|erase <phone-or-email>` answers GDPR Art. 15/17 requests (export / anonymize — parcels and audit history remain, unlinkable). Server logs mask contact details.
...
```bash
npm run test:detect   # detection, pickup policies, ID-scan classification, rate limiter, webhook HMAC, pure helpers (offline, 46 checks)
